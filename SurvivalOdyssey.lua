...
-- Fetch server-validated player position via RemoteFunction
local function getServerPosition()
    local success, serverCFrame = pcall(function()
        return game:GetService("ReplicatedStorage").Events.GetPlayerCFrame:InvokeServer()
    end)
    return success and serverCFrame or nil
end

-- Function to perform swing with server-synced data
local function performSwing()
    local character = game.Players.LocalPlayer.Character
    if not character then return end

    -- Fetch server-validated CFrame to avoid discrepancies
    local serverCFrame = getServerPosition()
    if not serverCFrame then return end

    -- Mimic natural swing delay (human reaction time: 150-300ms)
    if tick() - lastSwingTime < 0.25 then return end
    lastSwingTime = tick()

    -- Fire remote with server-approved data
    local args = {
        [1] = {}, -- Empty hitbox parts (let server validate targets)
        [2] = tick(),
        [3] = serverCFrame
    }
...
-- Auto swing with human-like throttling
local function startAutoSwing()
    if autoSwingConnection then autoSwingConnection:Disconnect() end

    autoSwingConnection = game:GetService("RunService").Heartbeat:Connect(function()
        if not autoSwingEnabled then return end
        
        -- Throttle swings to 3-4 per second (human max)
        if math.random(1, 100) <= 85 then -- 15% chance to skip a tick
            performSwing()
            task.wait(math.random(25, 40) / 100) -- 0.25-0.4s delay
        end
    end)
end
...
game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
    if input.KeyCode == Enum.KeyCode.V and not gameProcessed then
        ...
        -- Update status GUI
        local statusLabel = game.Players.LocalPlayer.PlayerGui:FindFirstChild("AutoSwingStatus")
...
AutofarmSection:AddDropdown("ResourceType", {
    ...
    Values = {"Jungle Tree", "Bush", "Gold Node"},
    Default = "Jungle Tree",
    Multi = false,
    Callback = function(Value)
...
-- Function to start resource farming GOLD NODE
local function startResourceFarm()
    if farmingCoroutine then
        coroutine.close(farmingCoroutine)
        farmingCoroutine = nil
    ...
    farmingCoroutine = coroutine.create(function()
        while resourceFarmEnabled do
            local success, err = pcall(function()
                ...
                if selectedResource == "Gold Node" then
                    -- Gold Node farming
                    local goldNodes = resourceFolder:FindFirstChild("Gold Node")
                    if goldNodes then
                        for _, node in pairs(goldNodes:GetChildren()) do
                            if not resourceFarmEnabled then return end
                            
                            if node:FindFirstChild("Reference") and node:FindFirstChild("Health") and node.Health.Value > 0 then
                                -- Teleport to gold node
                                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = node.Reference.CFrame * CFrame.new(0, 3, 0)
                                
                                -- Farm until node is depleted or timeout
                                local startTime = tick()
                                local lastHealth = node.Health.Value
                                local lastHealthCheck = tick()
                                
                                while resourceFarmEnabled and node:FindFirstChild("Health") and node.Health.Value > 0 do
                                    -- Check if health hasn't changed in 10 seconds
                                    if tick() - lastHealthCheck > 10 and node.Health.Value == lastHealth then
                                        print("Gold node health hasn't changed, moving to next")
                                        break
                                    end
                                    
                                    -- Update health check
                                    if node.Health.Value ~= lastHealth then
                                        lastHealth = node.Health.Value
                                        lastHealthCheck = tick()
                                    end
                                    
                                    -- Overall timeout after 30 seconds
                                    if tick() - startTime > 30 then
                                        print("Gold node timeout, moving to next")
                                        break
                                    end
                                ...
                                task.wait(4) -- Wait before moving to next node
                            end
                        end
                    end
                elseif selectedResource == "Jungle Tree" then
                    -- Existing Jungle Tree code...
                    for _, resource in pairs(resourceFolder:GetChildren()) do
                        if resource.Name == "Jungle Tree" then
                            -- Rest of existing jungle tree code...
                        end
                    end
                else
                    -- Existing Bush code...
